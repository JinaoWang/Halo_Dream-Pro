...
const commonContext = {
  ...
  initTimeCount() {
    try {
      console.log('=== 开始执行人生倒计时初始化 ===')
      console.log('timelife元素存在:', $('.timelife').length > 0)
      console.log('aside-timelife元素存在:', $('.aside-timelife').length > 0)
      console.log('当前时间:', new Date())
      console.log('DreamConfig存在:', typeof DreamConfig !== 'undefined')
      if (typeof DreamConfig !== 'undefined') {
        console.log('DreamConfig.countdown_name:', DreamConfig.countdown_name)
        console.log('DreamConfig.countdown_date:', DreamConfig.countdown_date)
      }

      if (!$('.timelife').length) {
        console.log('未找到timelife元素，跳过执行')
        return
      }
      if (!$('.aside-timelife').length) {
        console.log('未找到aside-timelife元素，跳过执行')
        return
      }
      console.log('找到所有必要元素，开始执行倒计时计算')
      ...
      const getTimeRemaining = () => {
        ...
        const getDifference = (unit) => {
          ...
          const total = isDay ? totalMs / (1000 * 60 * 60) + 1 : totalMs / (1000 * 60 * 60 * 24) + 1
          const passedMs = now - start
          const passed = isDay ? passedMs / (1000 * 60 * 60) : passedMs / (1000 * 60 * 60 * 24)
      ...
      if (DreamConfig.countdown_date && DreamConfig.countdown_date !== 'null') {
        let targetDate
        const today = new Date()
        const currentYear = today.getFullYear()
        
        // 处理datetime-local格式 (YYYY-MM-DDTHH:MM)
        let processedDate = DreamConfig.countdown_date
        if (processedDate.includes('T')) {
          processedDate = processedDate.split('T')[0]
        }
        
        console.log('处理后的日期:', processedDate)
        
        // 检查是否是月日格式 (MM-DD 或 MM/DD)
        const monthDayRegex = /^(0[1-9]|1[0-2])[-/](0[1-9]|[12]\d|3[01])$/
        if (monthDayRegex.test(processedDate)) {
          console.log('匹配月日格式')
          // 解析月日格式
          const [month, day] = processedDate.split(/[-/]/).map(Number)
          // 创建当前年份的目标日期
        ...
        countdownData = {
          name: DreamConfig.countdown_name || '倒计时',
        ...
        console.log('倒计时数据:', countdownData)
      }

      // 获取时间剩余数据
      const timeData = getTimeRemaining()

      console.log('时间剩余数据:', timeData)

      let htmlStr = ''
      // 使用card-content直接作为容器
      
      // 创建一个整合的容器，包含左侧自定义节日和右侧四个日期
      htmlStr += `
      ...
      if (countdownData) {
        htmlStr += `
						<div class="count-right">
							...
							<span class="name">${countdownData.name}</span>
							<span class="time">${countdownData.days}</span>
							<span class="date">${countdownData.date}</span>
						</div>`
      }
        ...
        htmlStr += `
							<div class="count-item">
								...
								<div class="item-progress">
									...
									<span class="percentage ${parseFloat(item.percentage) >= 46 ? 'many' : ''}">${item.percentage}%</span>
									<span class="remaining ${parseFloat(item.percentage) >= 60 ? 'many' : ''}">
										<span class="tip">还剩</span>
										${Math.floor(item.remaining)}
										<span class="tip">${item.name === '今日' ? '小时' : '天'}</span>
									</span>
								</div>
							</div>`
      })
      ...
      console.log('生成的HTML:', htmlStr)
      
      $('.aside-timelife').html(htmlStr)
      
      // 添加鼠标悬停事件
      $('.count-down').on('mouseenter', function() {
        $(this).find('.count-left .remaining').css('opacity', '1')
        $(this).find('.count-left .remaining').css('transform', 'translateX(0)')
        $(this).find('.count-left .percentage').css('opacity', '0')
        $(this).find('.count-left .percentage').css('transform', 'translateX(-10px)')
      }).on('mouseleave', function() {
        $(this).find('.count-left .remaining').css('opacity', '0')
        $(this).find('.count-left .remaining').css('transform', 'translateX(10px)')
        $(this).find('.count-left .percentage').css('opacity', '1')
        $(this).find('.count-left .percentage').css('transform', 'translateX(0)')
      })

      console.log('=== 人生倒计时初始化完成 ===')
    } catch (error) {
      console.error('=== 人生倒计时初始化出错 ===', error)
    }
  },
  /* 安全链接 */
  initSecurityLink() {
